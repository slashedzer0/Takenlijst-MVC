...
class TodoService:
    ...
    async def create_task(task: TodoIn) -> TodoOut:
        ...
        task_id = count + 1
        doc = {"task_id": task_id, "task": task.task, "status": 0}
    ...
    @staticmethod
    async def get_task(task_id: int) -> TodoOut:
        task = await database.tasks.find_one({"task_id": task_id}, {"_id": False})
        if task:
            return TodoOut(**task)
        return None
